...
## 页数的统计口径（2026-09-04 跑题者反馈：多处混用）

**统一口径：一切页数要求都指「PDF 总页数」**，除非赛制规范另有明文。

- 报告页数时写清是总页数还是正文页数，并注明统计方式（`pdfinfo` 输出）。
...
## 三向审计的默认命令与排除规则（2026-09-04 跑题者反馈）

分节论文下，`audit_numbers.py` 若把 `main.tex` 与 `numbers.tex` 也当作正文扫描，
会把宏定义本身与装配文件里的占位数字当成裸数字，制造大量伪阳性。默认命令固定为：

```bash
python scripts/audit_numbers.py --ledger 结果/results_ledger.json     --numbers 论文/numbers.tex --root . --out 结果/审计报告.md     --tex <所有分节 tex，排除 main.tex 与 numbers.tex>
```

- 排除 `main.tex`：它只做装配，不含正文数字。
- 排除 `numbers.tex`：它是宏定义源，被它自己审计属于自指。
- 其余分节 tex 全部传入；已递归展开时用 `--no-expand` 防止重复计入。
- 报告里的「待裁定裸数字」必须**逐条处置并记录裁定理由**，不得只统计数量。
