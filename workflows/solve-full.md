...
单人也能跑完，但 Modeler、Critic、Auditor 默认使用未参与其审查对象创作的新会话。
若只能由参与过被审阶段的同一会话复核，审查记录必须标记 `independence: degraded`，
不得声称完成了独立审查；**交接点的产物一个都不能省**。
...
6. `python scripts/prd_gate.py 结果/PRD.md --out 结果/gates/G-0-PRD.md`
   ——待定项必须清零，完成标准逐条有编号，并给出反引号包裹的判定命令；退出码必须为 0。
...
**输入证据**：题面、Coordinator 完成的 PRD 与建模详要。此时还没有代码或计算结果。
**核心问题**：拟议规格是否可执行且在科学上站得住——题意、方法族、可识别性、承重假设、
主路线与独立核验路线、数据可得性、完成标准和验证方案逐项核对。

1. 只复核 Coordinator 提出的规格，不另造路线，也不动手实现或试算。
2. 有异议时交出逐条异议清单，退回 Coordinator 修订 PRD 与建模详要；修订后必须再次复核。
3. 只有明确交出「无异议，可执行」，Coordinator 才能定稿，Coder 才能进入 P0。

**交出与退回**：异议清单退回 Coordinator，或交出「无异议，可执行」。
**禁止越权**：不得自行改写规格后直接下发，不参与 P2 执行，也不审查尚不存在的运行结果。
...
1. 按 Coordinator 定稿且经 Modeler 放行的 PRD 与建模详要推进；多问题目共用同一个判定内核，
...
2. 查方法卡拿候选：`python scripts/method_query.py "<题意关键词>" --top 6`。
   ...
   可选调用 `math-model-resource-router` 查找有维护、可钉版本的算法实现；资源不可用时在 `选型.md` 记录 `N/A` 与定量原因，照常自研，不阻断流程。
3. `选型.md` 每问写一行实现来源：库与 commit ／ 自研理由 ／ N/A。优先用有维护、
...
**输入证据**：Coder 交出的实际代码、运行结果、测试与红绿记录、门禁记录、证书、
复现材料、规模与复杂度实测，以及 PRD、建模详要和结果主张。
**核心问题**：这些实际产物能否复现，是否与声明一致，并经得住反例、极端和退化情形。

1. **复现**：换机器、换种子、解包后照说明跑，能否得到同一结论。
2. **红绿**：每个关键断言是否先红后绿，expected 是否独立于被测实现。
3. 声明-实现一致性抽查：`规格或结果主张 ↔ 代码文件:行号 ↔ 该行实际谓词`，每问关键判据至少抽 3 处。
4. 跨问一致性从代码行号取证；私货约束逐条回指题面原文。
5. 用实测核规模、复杂度和运行预算，并与解析界、粗模型或同领域尺度律做量级对照。
6. 用反例、极端和退化情形尝试击穿结论；逐问核声明强度、gap、误差预算与证据强度。
7. 核验官方门禁命令、退出码、证书与复现材料，不能让自写替代检查给自己发通过。
8. 内核或输入改变后 `python scripts/ledger.py --stale-check 结果/results_ledger.json --write` 并回 P3。

完整维度见 `references/roles.md` 的 Critic 表（R1–R10）。
**交出与退回**：给 Coder 可定位到证据的缺陷清单；修正实际产物后重跑受影响门禁。
若缺陷暴露规格错误，退回 Coordinator 回滚规格，并重新经过 Modeler 放行。
**禁止越权**：Critic 的唯一正常入口是 P4，不参与 P2 或执行前规格复核，也不直接替 Coder 修改产物。
