...
**入口是两问，但不止两问。** 这两条最容易查、也最容易被跳过：

- **复现了吗？** 换台机器、换个种子、解包后照说明跑，还能得到同一个数吗？
- **红绿了吗？** 每个断言都先红过吗，还是写完实现才补的测试？

往下继续拷问，直到追到不可再分的前提为止：

- **这个前提是谁定的**——题面原文，还是我们自己顺手假设的？
- **如果这条定错了，哪个可观测量会变**？挂不上观测量的判断是不可证伪的意见。
- **我在推导还是在类比**？「同类题都这么做」不是论证。
- **这个惯例在本题成立吗**？惯例有它的前提，前提变了惯例就失效。
- **有没有更简单的做法被跳过了**？复杂方案要能说出简单方案为什么不够。
- **查过的和记得的，界线在哪**？

完整维度见 `references/roles.md` 的 Critic 表（R1–R8）。
