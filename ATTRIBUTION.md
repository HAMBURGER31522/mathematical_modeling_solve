...
| 上游项目 | 许可 | 萃取内容 |
|---|---|---|
| [sweetcornna/mathodology](https://github.com/sweetcornna/mathodology) | MIT | 图文质检思想（contact sheet/逐张质检/PDF 检查）、多路线选型比较、decision memo |
...
- [i3by4t3oyt/Mrite](https://github.com/i3by4t3oyt/Mrite) @ `713aa65`（含 LICENSE 文件）：逐字复制其 `projects/高教社杯/论文/` 的 `format.cls` 与分节 TeX 模板到 `assets/paper/`，并保留摘要、表格、图形和算法段落的原始排版结构。四处有意差异为：模型检验章改为六小节；模型评价章缺点按「缺陷—影响—改进」写；附录加 `\label{sec:appendix}` 供页数切分；`main.tex` 增加 `numbers.tex` 注入位并在缺失时输出 `[NUMBERS-MISSING]`。
...
- `scripts/` 下的全部门禁执行体与契约测试。
  （`assets/paper/` **不在原创之列**——它逐字引自 Mrite，见上文；本仓只在其上做了四处有意差异。）

## 字体来源与许可

`assets/paper/` 下的字体文件（`Source Han Serif CN` Bold/Regular）来自 Adobe 的思源宋体项目，
不是 Mrite 仓库的内容；随字体分发完整的 SIL Open Font License 1.1 正文（见 `fonts/OFL.txt`）。
...
- [math-model-resource-router](https://github.com/HAMBURGER31522/Mathematical_Modeling_Algorithm)（用户自有算法资源路由库）：P2 实现层选库的首选依据——90 个经审计的 GitHub 算法源（钉定 commit、入口路径、许可证据、声明缺口）。查找与降级顺序见 [`workflows/solve-full.md` 的 P2](workflows/solve-full.md)；不可用时记 N/A 照常自研，功能不受阻断。
