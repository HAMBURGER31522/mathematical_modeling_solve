...
- [Rzna-5559/Mrite](https://github.com/Rzna-5559/Mrite)：仓库无 LICENSE 文件，仅 README 声明 MIT 而无授权正文，从严按"仅参考做法"处理（证据快照见融合仓库 scorecards/license-evidence-mrite.md，含访问日期）。参考的做法：求解计划先行、两阶段执行（先算后画）、xelatex 编译错误门禁、排版优化循环、摘要页数 aux 验证、**分节 tex + 主控 \input 装配的论文工程形态**（该形态同为基线 A 论文的既有结构；`assets/paper/` 的全部文件内容为原创撰写，未复制上游任何文本）；**分节模板的密度契约**（步骤式推导 + 每步公式链下限、参数表附「选取依据」列、模板内预置图位与"由表X可知／从图Y可以看出"的结果分析句式、章末承上启下句、检验章的误差表与灵敏度表骨架）。上述均为**做法参考**，`assets/paper/` 内全部文字为原创撰写，未复制上游任何文本。
...
- [figure-forge](https://github.com/HAMBURGER31522/create_image_fukinggood)（MIT，用户自有作图库）：P5 出图的候选实现层——期刊级 recipe 模板、论点合同、硬拒绝清单、run_qa 出图自检。当前按 A/B 对比协议与内置生成规范并行评估，升格为默认的条件见 references/figures-standards.md 首节；不可用时全部手写，功能不受阻断。
